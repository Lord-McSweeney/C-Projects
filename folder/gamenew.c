...
char binds[256] = {'U', 'N', 'N', 'N', 'D', 'W', 'W', 'W', 'R', 'E', 'E', 'E', 'H', 'H', '\0'};
...
int main() {
  FILE *fstr = fopen("./keybindsp.txt", "r");
  if (access("./keybindsp.txt", F_OK)) {
  ...
  if (keybindStatus == 1) {
    ...
    printf("Successful reading of keybindsp.txt.\n");
  }
  printf("(Resize terminal window to 180x8)\n");
  while(1) {
    resetRows(px, py);
    printf("\n");
    printRows();
    ...
    for (i = 0; i < strlen(binds); i = i + 2) {
      if (input[0] == binds[i]) {
        ...
        if (k == 'H') {
          ...
          if ((getCharAt(px, py - 1) == 32) && (getCharAt(px, py + 1) != 32)) {
            py -= 1;
            printf("You jumped.\n");
          }
          if (getCharAt(px, py - 1) == 64) {
            printf("\nYou won the game!\n");
            py -= 1;
            level ++;
          }
          if (!(getCharAt(px, py - 1) == 32)) {
            printf("There was a wall above you.\n");
          }
        } else if (k == 'W') {
          if (getCharAt(px - 1, py) == 32) {
            px -= 1;
            printf("You went leftwards.\n");
          } else if (getCharAt(px - 1, py) == 64) {
            printf("\nYou won the game!\n");
            px -= 1;
            level ++;
          } else {
            printf("There was a wall to the left of you.\n");
          }
        } else if (k == 'E') {
          if (getCharAt(px + 1, py) == 32) {
            px += 1;
            printf("You went rightwards.\n");
          } else if (getCharAt(px + 1, py) == 64) {
            printf("\nYou won the game!\n");
            px += 1;
            level ++;
          } else {
            printf("There was a wall to the right of you.\n");
          }
        } else {
          printf("Unknown bind. Please bind keys to N (jump), W (move left), E (move right), or H (help).\n");
        }
      }
    }
    if (currentTick < 0) {
